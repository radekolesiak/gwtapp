...
public class UserRegisterRpc extends RemoteServiceSession implements
	...
	public TUserRegisterValue processUserRegister(
		...
		if (!userRegister.hasErrors() && confirm != null) {
			String smtp = getBean(IPropertyModel.class).get(
					Properties.MAIL_SMTP_SERVER).getValue();
			String noreply = getBean(IPropertyModel.class).get(
					Properties.MAIL_USER_NOREPLY).getValue();
