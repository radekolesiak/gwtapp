...
public class ClientModule extends AbstractGinModule {
	...
	protected void configure() {
		...
		bind(ListBox.class).annotatedWith(ListPanel.Bind.class)
				.to(ListBox.class);

		bind(String.class).annotatedWith(ListPanel.Bind.class)
				.toProvider(ListPanelFieldName.class);

		bind(new TypeLiteral<ListPanel.Formatter<ListPanelTestManual.Item>>() {
		}).annotatedWith(ListPanel.Bind.class).to(
				ListPanelTestManual.Formatter.class);

		bind(new TypeLiteral<List<Item>>() {
		}).annotatedWith(ListPanel.Bind.class).toProvider(
	...
	private void bindReCaptchaUserRegister() {
		bind(ReCaptchaUser.class).annotatedWith(
				ReCaptchaUserRegisterPanel.Bind.class).to(
